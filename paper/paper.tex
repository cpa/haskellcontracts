...
The syntax of $\hprime$ is defined in figure~\ref{hprime-stx}. A
...
\note[nc]{
Not true that we only consider full application of functions: the definition of satisfaction for arrow contracts
...
\begin{itemize}
\item $(\phi_\text{injective})$ Constructors are injective.  That is, for
  each term constructor $K$,
  if $K e_1 \cdots e_n = K e'_1 \cdots e'_n$, then $e_i = e'_i$ for
  all $i$.  We axiomitize this via projections $\proj{i}{K}$: for each $i$,
  $\proj{i}{K}(K e_1 \cdots e_n) = e_i$.
...
\item $(\phi_\text{cf})$ Then, we have to give crash-freeness conditions for each $K_i$:
  ...
  \item $\gets$: if we pack crash-free values in a data constructor,
    the resulting value is crash-free.  For $K$ has arity $n$
    we assert $\ctrans{K \in \cfc^n \to \cfc}^-$.
...
$$\ttrans{\mbox{data } T = K_1 ~a_1, \dots, K_n ~a_n} = \phi_\text{injective} \land \phi_\text{disjoint} \land \phi_\text{cf} \land \phi_\text{lazy}$$
 \hspace{5.6cm}where
  ...
  \caption{$\ttrans{}$ -- Data type translation\note[nc]{Need
      $\min{a}$ to be sufficient for unfolding in
      $\phi_\text{injective}$ to get invertability of constructors!
      I thought we needed $\min{\proj{j}{K_i}(a)}$ to be sufficient when
      we use a projection based translation of case expressions, but
      as Koen pointed out this is not true.
      }}
  \label{ktrans}
\end{figure*}
