...
it both to guide proof search and guarantee (?)\footnote{
  % 
  Do we really guarantee finite models?  I think Koen said ``yes'',
  but I don't see how to prove this.  Namely, I don't see how to prove
  we never have $S : M \to M$ for $M \subseteq \{ x \mid \min{x} \}$ and
  $M \neq \emptyset$.

  I think I wrote down an example where we have $\min{Z}$,
  $\min{S~Z}$, $\min{S~(S~Z)}$, etc, but it seems I'd need an
  induction axiom to prove that $S$ really took $M$ to $M$, for $M$
  the naturals.
  % 
} the existence of
...
The $\min$ predicate complicates the translation of contracts,
requiring us to define separate contract translations for goals and
axioms.  The only actual difference is for predicate-contract, but we
have considered many minor variations.
...
\item $(\phiLazy)$ No full application of a constructor $K_i$ is
  unreachable or crashing.  In other words, term constructors don't
  scrutinize their arguments / are lazy.
\end{itemize}
...
positions in an implication
\[
- \to +.
\]
The goal of the $\min$'s twofold: 
...
negative, and contract goals as positive. So, the theory
consists of $\ctrans{}^-$ axioms, so call it $\Gamma^{\in^-}$.
We want to prove $\Gamma^{\in^-} \vdash \ctrans{e \in c}^+$.  
But that's equivalent to $\Gamma^{\in^-},\lnot\ctrans{e \in c}^+ \vdash \bot$,
and that is the refutation we feed to the theorem prover.

An example: 
given $\ctrans{g \in \cfc \to \cfc}^-$, we may 
...
then using $\phiCF$ for $Z$, which is $\ctrans{Z \in \cfc}^-$, 
i.e. $\min{Z} \to \cf{Z}$, exactly what we need.
...
\subsection{Case translations}
XXX: describe the various case translations here.
...
It's possible to encode, prove, and apply lemmas in our system,
without adding any extensions.  The application of lemmas can change
the termination behavior of programs (although so far I think we've
only used lemmas in other lemmas), and it's up to the user to look out
for this.  

Before presenting the solution we have to understand the problem: the
FOL prover requires finite axiomitizations, and hence there is no
general induction support (FOL does not allow quantification over
formulas, and so induction can only be axiomitized with a schema).
Each inductive proof of a universal property $\forall x. \phi(x)$
requires its own induction axiom, and the system does not attempt to
discover these (a separate problem).  When checking the contract of a
recursive function, we axiomitize induction for that function,
following the structure of its recursion.  But a contract may depend
on a lemma, itself requiring an inductive proof, and so we need a
facility for introduces auxiliary inductions.  Of course, we also need
to be able to state the lemmas themselves, because the system does not
discover them.

Our solution is to define ``dummy'' recursive functions, whose
recursion structure follows the induction needed to prove the lemma.
We don't actually care what these functions compute, so we define a
unit type, $QED$, and return that in the base cases.  E.g., if we want
to prove that $Z$ is a right identity for addition, then we can define
a dummy function \texttt{lem\_add\_Z\_r}
\begin{verbatim}
...
\end{verbatim}
and give it a contract stating the property in question
\begin{verbatim}
lem_add_Z_r ::: x:CF -> CF&&{qed: x `eqNat` (x `add` Zero)}
\end{verbatim}

To use the lemma we, need to have $\min$ on it's instantiation.  To
get this we introduce another dummy function, $using$,
s.t. $e `using` l$ scrutinizes $l$ and returns $e$.  I.e.
\begin{verbatim}
e `using` lem = case lem of
  QED -> e
\end{verbatim}
The \texttt{case} match in $using$ introduces $\min$ for the lemma,
allowing it's contracts to be unfolded.

XXX: Koen has a long and detailed writeup of lemmas in the his google
doc.

XXX: best examples are \texttt{egs/yes/add-symmetric.hs} and
\texttt{egs/Lem/EqNatTrans.hs}.
...
$$\forall g,x.~\cf{g} \land \cf{x} \to \cf{g~x}$$
\caption{Encoding of higher-orderness.
%
XXX: need 'min's.
%
XXX: i don't think the second and third formulas are implemented.}
\label{ho-fig}
\end{figure*}
