...
eTrans (H.Sat e c) = do 
  ts <- undefined --sTrans (H.Var "x") c TODO
...
dTrans (H.LetCase f vs e pes) = do
  ...
  let vvs = map (F.Var . F.Regular) vs
      ...
      eq11 = (F.And $ (F.Not $ et :=: F.Var F.BAD):bigAndSel ) :=>: eq12
      eq12 = (F.FullApp (F.Regular f) vvs :=: F.Var F.UNR)
      bigAndSel = [F.Not $ et :=: F.Weak (F.FullApp (F.Regular di) [F.FullApp (F.Regular ("sel_"++(show i)++"_"++di)) [et] | i <- [1..ai]]) | (di,ai) <- arities]
      fptr1 = (F.Forall vvs $ (F.And [F.CF v | v <- vvs]) :=>: F.CF (F.FullApp (F.Regular f) vvs)) :<=>: (F.CF $ F.Var $ F.Regular (f++"_ptr"))
      fptr2 = F.Forall vvs $ (F.FullApp (F.Regular f) vvs) :=: (F.App $ (F.Var . F.Regular) (f++"_ptr") : vvs)
      fptr3 = F.Forall vvs $ (F.FullApp (F.Regular (f ++ "p")) vvs) :=: (F.App $ (F.Var . F.Regular) (f++"p_ptr") : vvs)
...
sTrans e (H.Pred x u) =  do
  ...
  let a = prefix s
      ...
      fs = fofBag s
  put $ s { fofBag = [] }
  return $ F.And $ [F.Or [(et :=: F.Var F.UNR) ,F.And [(F.Not $ (F.Var F.BAD) :=: ut') , F.Not $ ut' :=: (F.Var $ F.Regular "false")]]]++fs -- The data constructor False.
...
s2D ((d1,a1,c1),(d2,a2,c2)) = do
  ...
  return $ F.Forall (map (F.Var . F.Regular) (xs1 ++ xs2)) $ F.Not $ (F.FullApp (F.Regular d1) (map (F.Var . F.Regular) xs1)) :=: (F.FullApp (F.Regular d2) (map (F.Var . F.Regular) xs2))
...
s4D (d,a,c) = do
  ...
  if xs /= [] 
    then (return $ F.Forall (map (F.Var . F.Regular) xs) $ F.Not (et :=: (F.Var F.UNR)))
    else return $ F.Not ((F.Var $ F.Regular d) :=: F.Var F.UNR)
