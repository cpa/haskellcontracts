...
dTrans (H.Let f vs e) = do
  ...
  where vvs = map (F.Var . F.Regular) vs
        fptr1 = F.Forall vvs $ F.Iff (F.Implies (F.And [F.CF v | v <- vvs]) $ F.CF (F.FullApp (F.Regular f) vvs)) (F.CF $ F.Var $ F.Regular (f++"_ptr"))
...
sTrans e (H.AppC x c1 c2) = do
  ...
  f2 <- case e of 
    H.Var x -> sTrans (H.FullApp x [H.Var $ freshX]) c2'
    H.FullApp x xs -> sTrans (H.FullApp x (xs++[H.Var $ freshX])) c2'
