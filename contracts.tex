...
\begin{center}
\begin{array}{rclr}
  u,e &:=& x \mid f \mid e~e \mid D(e,\dots,e) \mid \bad \mid n \mid e \mbox{ `satisfies` } c \mid \cf{e} \mid f(e,\dots,e) & (Expression)\\
  p &:=& \Delta,p \mid T,p \mid f \in c,p \mid \epsilon & (Program)\\
  \Delta &:=& d \mid opaque(d) & (Defintion~and~scope)\\
  d &:=& f~x_1 \dots x_n ::: c = e\mid f~x_1 \dots x_n ::: c = \mbox{ case } e \mbox{ of } [(pat_i,e_i)] & (Definition)\\
  T &:=& \mbox{data } x = DCons  & (Data~type~definition)\\ %TODO make type clearer
...
&\cup& (\forall x_1 \dots x_n. (\bigwedge \cf{x_i} \rightarrow \cf{f(x_1,\dots,x_n)}) \leftrightarrow \cf{f\_ptr}\\
&\cup& (\forall x_1 \dots x_n. (f(x_1,\dots,x_n) = app(...app(f\_ptr,x_1)...,x_n)))\\
...
Equation (11) and (12) state that if we have a type mismatch than the code is $\unr$. It should never occur because we're assuming that the code has already been typechecked. %TODO eq number
