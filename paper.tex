...
to an exception) and that $Cons(x,y)$ \add[nc]{never} crashes iff \change[nc]{either}{both} $x$ \change[nc]{or}{and} $y$
\add[nc]{never} crash. The statement $x$ \add[nc]{never} crashes is encoded by the term $\cf{x}$\note[nc]{Made more precise in section \ref{sec:cf}}.
...
because we have $\cf{xs}$ and \annote[nc]{$\lnot \cf{\bad}$}{Obviously true, given the definition of $CF$, but it's worth noting that this plays a key part in the axiomitization of $CF$, which is ommited in this draft (so far ...)}. The constraint
