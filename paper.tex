...
\item $(\phi_\text{cf})$ Then, we have to give crash-freeness conditions for each $K_i$:
  ...
  \item $\gets$: if we pack crash-free values in a data constructor,
    the resulting value is crash-free. \note[nc]{We use the contract
    translation for arrows, and this translation is principled.}
...
$$\ktrans{\mbox{data } T = K_1 ~a_1, \dots, K_n ~a_n} = \phi_\text{projection} \land \phi_\text{disjoint} \land \phi_\text{cf} \land \phi_\text{lazy}$$
 \hspace{5.6cm}where
  ...
  \caption{$\ktrans{}$ -- Data type translation\note[nc]{Need
      ...
      $\phi_\text{projection}$ to get invertability of constructors!
      On the other hand, in a translation of case expressions based on
      projectors, we probably need $\min{sel_{j,K_i}(a)}$ to be
      sufficient for unfolding.  So, we make both sufficient in the
      implementation: $\min{a} \lor \min{sel_{j,K_i}(a)}$.}}
  \label{ktrans}
\end{figure*}
