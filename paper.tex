...
$$\ktrans{\mbox{data } T = K_1 ~a_1, \dots, K_n ~a_n} = \phi_\text{projection} \land \phi_\text{disjoint} \land \phi_\text{cf} \land \phi_\text{lazy}$$
 \hspace{5.6cm}where
  \begin{center}
    ...
    \end{array} \]
  \end{center}
  \caption{$\ktrans{}$ -- Data type translation\note[nc]{Need $\min{a}$ to be sufficient for unfolding in $\phi_\text{projection}$ to get invertability of constructors!}}
  \label{ktrans}
\end{figure*}
