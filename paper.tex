...
\item Until \ref{ho} we will only consider full application of
...
\begin{figure*}
\begin{eqnarray*}
  \strans{e \in \{x \mid b(x) \}} &=&  \min{\etrans{b(e)}} \land (\etrans{b(e)} = true \lor \etrans{b(e)} = \unr)\\
  \strans{e \in x:c_1 \to c_2(x)} &=& \forall x.~\min{\etrans{f(x)}} \to (\strans{x \not \in c_1} \lor  \strans{\etrans{e(x)} \in c_2(x)})\\
  \strans{e \in \cfc} &=& \cf{e}
\end{eqnarray*}

\begin{eqnarray*}
  \strans{e \not \in \{x \mid b(x) \}} &=&  \min{\etrans{b(e)}} \land (\etrans{b(e)} = false \lor \etrans{b(e)} = \bad)\\
