...
definition).  On the other hand, if we define
\protect \[
\ctrans{g \in x:c1 \to c2} 
...
$\ctrans{x \in c1}$ (i.e. $\lnot \min{Z} \lor \cf{Z}$).
...
from $\ctrans{g \in \cfc \to \cfc}^-$ (note the negative).
...
\min{x}$, which is not true a priori.  But, it's easy to add the $\min{g ~x}$ 
assumption to the positive arrow translation to recover the
 ``negative implies positive'' property.

Going back to the current translation, we consider some of it's
...
\item We'd like to have a result  like
  \protect \[
  \text{Given some validity or consistency assumptions on $\Gamma$ and $e \in c$}, 
