...
We use first-order logic with equality, defined in figure \ref{fol-stx}.\note[nc]{I guess ``with equality'' means that the proof theory has an axiom schema for substition, i.e. forall $phi$, $\forall x,y.x=y \to phi \iff phi[y/x]$, axioms making $=$ an equality relation, and the requirement that all models interpret $=$ as true equality on the carrier.}
...
\item $(\Phi_3)$ Then, we have to give crash-freeness conditions for each $K_i$:
  ...
  \item $\rightarrow$: a value $t$ of type $T$\note[nc]{
    ``type'' is used intuitively here: it means the constructor head is of type $T$.
...
[[e in {x|p}]]       := min(p[e/x]) /\ (e = unr \/ p[e/x] = true)
not[[e notin {x|p}]] := min(p[e/x]) -> (e /= bad /\ p[e/x] /= false)
...
[[e in {x|p}]]       := min(p[e/x]) /\ (e = unr \/ p[e/x] in {true,unr})
not[[e notin {x|p}]] := min(p[e/x]) -> (e = unr \/ p[e/x] in {true,unr})

-- what we actually discussed was adding a top level min(e) assumption, but
...
c := {x|x==True} -> CF
[[e in c]]+ = forall x. min(e x) -> [[x in {x|x==True}]]- -> [[e x in CF]]+
            = forall x. min(e x) -> (x = unr \/ p in {true,unr})
                                 -> (min(e x) -> CF(e x))
...
and D[[]] like
  ...
  (x = Nil /\ f x = eNil)
  \/
  (x = Cons(cons_1 x,cons_2 x) /\ f x = eCons[cons_1 x/y, cons_2 x/ys])

the latter is nice because we don't need to invent names (consider
...
\add[nc]{We also state that $\unr$ is crash-free: $\cf{\unr}$.}\note[nc]{
