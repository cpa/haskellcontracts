...
For an overview of the different translations we define, see \ref{trans}
...
\begin{figure}
  ...
  \begin{array}{rcl}
    ...
    \dtrans{f~\vec{x} = \dots} &\to& \Phi~(\mbox{Set of formulae}) \\
...
\begin{figure*}
  ...
  \begin{center}
    \begin{array}{rclcl}
      \forall \vec{a},y.~\min{y} \land f(\vec{a}) = y &\to& (\multicolumn{3}{c}{\min{e} \land (e = \bad \lor \bigvee_i \forall \vec{x}.~e = K_i(\vec{x}) \lor y = \unr)} \\      
...
We break down the translation for datatypes in four parts, described in \ref{ktrans}
...
We give in \ref{ctrans} the translation of contract satisfaction.
