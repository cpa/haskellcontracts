...
&&      :=\forall x_1, \ldots, x_n. \min{f(x_1, \ldots, x_n)} \\
...
\begin{figure}
 \begin{center}
  \[  \begin{array}{rcl}
    \dtrans{def} &\to& \phi~(\mbox{formulae}) \\
    \ktrans{\mbox{data } T = \dots} &\to& \phi~(\mbox{formulae}) \\
    \ctrans{f \in c} &\to& \phi~(\mbox{formulae}) \\
  \end{array} \]
  \end{center}
...
\begin{array}{rcl}
  ...
  f(\vec{x})) \to a = e \\
   \raisebox{ 0.5 cm }{$\dtrans{f(\vec{x}) = \mbox{case } e \mbox{ of } [K_i(\vec{x_i}) = e_i]}$} &\raisebox{0.5cm}{=}&  \hspace{-0.18cm} \begin{array}{rclcl}
     ...
     &\land& \forall \vec{x_1}.~e = K_1(\vec{x_1}) &\to& a = e_1 \\
     &\land& & \cdots & \\
     &\land& \forall \vec{x_n}.~e = K_n(\vec{x_n}) &\to& a = e_n \\
   \end{array} 
\end{array}
\]
...
\item $(\phi_\text{disjoint})$ For each pair of constructors $K_i,K_j$, $i \not = j$, we state that they can
...
$$\ktrans{\mbox{data } T = K_1 a_1, \dots, K_n a_n} = \phi_\text{projection} \land \phi_\text{disjoint} \land \phi_\text{cf} \land \phi_\text{lazy}$$
 \hspace{5.6cm}where
  \begin{center}
    \[  \begin{array}{rcl}
      \phi_\text{projection} &=& \bigwedge_{1 \leq i \leq n}~\forall \vec{x},a.~(\min{a} \land a = K_i(\vec{x})) \to \bigwedge_{1 \leq j \leq a_i} x_j = sel_{j,K_i}(a) \text{\note[nc]{??? $\min{a}$ or $\min{sel_{j,K_i}{a}}$?}}\\
      \phi_\text{disjoint} &=& \bigwedge_{1 \leq i < j \leq n}~\forall \vec{x},\vec{y},a.~ \lnot (\min{a} \land a = K_i(\vec{x}) \land a = K_j(\vec{y})) \\
      \phi_\text{cf} &=& \bigwedge_{1 \leq i \leq n}~\forall \vec{x},a.~(\min{a} \land a = K_i(\vec{x})) \to ((\bigwedge_{1 \leq j \leq k} \cf{x_j}) \tot \cf{a}) \\
      \phi_\text{lazy} &=& \bigwedge_{1 \leq i \leq n}~\forall \vec{x},a.~(\min{a} \land a = K_i(\vec{x})) \to a \neq \unr \land a \neq \bad 
    \end{array} \]
  \end{center}
